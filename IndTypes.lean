...
/-
Or what are often call sum types; akin to a disjoint union from two sets
-/
...
variable (α β : Type) 
variable (x : Notes.Sum α β)

example (a₀ : α) : Notes.Sum α β := .left a₀
...
#check 1 + 2 
#check (1 + 2 : Int) /-Cast a natural number as an integer-/
#check #[] /-Gives an array-/
